...
import SecretVault, {
  SecretVaultInline,
} from "../components/features/SecretVault";

const _scImgs = import.meta.glob(
  "../assets/secret/sc*.{jpg,jpeg,png,webp,gif}",
  { eager: true },
);
const _scVideos = import.meta.glob("../assets/secret/sc*.{mp4,webm,mov}", {
  eager: true,
  query: "?url",
  import: "default",
});
...
export default function SecretCat() {
  ...
  const handleMeow = (e) => {
    setMeowCount((c) => c + 1);
    try {
      if (!meowAudio.current) meowAudio.current = new Audio("/meow.mp3");
      meowAudio.current.volume = 0.7;
  ...
  const vaultVideo =
    vaultConfig?.video && !vaultConfig.video.includes("PASTE")
      ? vaultConfig.video
      : null;
        ...
        >
          ...
          <motion.h1
            ...
            style={{
              ...
              textShadow: "0 2px 20px rgba(63,168,224,0.2)",
            }}
          >
